...
def visualize_classifier(classifier, X, y, title=''):
    # Определение минимального и максимального значений для X и Y,
    # которые будут использованы в сетке
    min_x, max_x = X[:, 0].min() - 1.0, X[:, 0].max() + 1.0
    min_y, max_y = X[:, 1].min() - 1.0, X[:, 1].max() + 1.0

    # Определение шага сетки для построения графика
    mesh_step_size = 0.01
